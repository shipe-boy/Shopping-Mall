...
<template>
  <div>
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="(item,index) in goodsCart" :key="index">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'checked': item.checked == 1}"
                  ...
                  <div class="cart-item-pic">
                    <img :src="'http://localhost:3000' + item.productImg" :alt="item.productName">
                  </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delGoodsConfirm(item)">
    ...
    <modal :mdShow="mdShow" @close="this.mdShow = false;">
      <p slot="message">您确定要删除该商品吗</p>
      <div slot="btn-group">
        <a class="btn btn--m" href="javascript:;" @click="delGoods">确定</a>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      checkedAllFlag() {
        return this.checkedCount == this.goodsCart.length;
      },
      totalPrice() {
        let money = 0;
        this.goodsCart.forEach(item => {
    ...
    methods: {
      getGoodsCart() {
        this.$ajax('/users/goodsCart')
          .then(res => {
            // console.log(res)
            if (res.data.status == 1) {
              this.goodsCart = res.data.result;
            }
          }).catch(err => {
            console.log(err)
          })
      },
      delGoodsConfirm(item) {
        this.delItem = item;
        this.mdShow = true;
      },
      delGoods() {
        ...
        this.$ajax.post('/users/cartDel', {
          ...
          if (res.data.status == 1) { //删除成功
            this.mdShow = false;
            this.getGoodsCart();
            this.$store.commit('cartCount', -this.delItem.productNum)
          }
        }).catch(err => {
      ...
      checkedAll() {
        ...
        this.$ajax.post('/users/checkedAll', {
            checkedAll: flag
          })
          .then(res => {
            // console.log(res)
