      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addressListFilter" :key="index" :class="{'check': index === checkIndex}" @click="checkIndex = index;selectAddressId = item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
                    <dd class="address">{{item.adress}}</dd>
                  ...
                  <div class="addr-opration addr-set-default">
                    <a v-if="!item.idDefault" href="javascript:;" class="addr-set-default-btn" @click="setDefault(item.addressId)"><i>Set default</i></a>
                  </div>
                  <div v-if="item.idDefault" class="addr-opration addr-default">Default address</div>
                </li>
                <li class="addr-new">
          ...
          <div class="lemall-msg-info hidden">
            <span>The region you selected is not within our delivery area. Please select another shipping address within
      ...
      getAddressList() {
        this.$ajax.get('/users/address').then(res => {
            ...
            if(res.data.status == 1){
                ...
                this.selectAddressId = this.selectAddressId ? this.selectAddressId:this.addressList[this.checkIndex].addressId
            }
        }).catch(err => {
          console.log(err);
